...
Heimdall polls Linear for issues entering an active state, then:
...
5. Polls GitHub for PR comments on Heimdall-managed pull requests so it can refine specs or run `/opsx-apply` with an allowed agent
...
HEIMDALL_LINEAR_API_TOKEN=your-linear-token
HEIMDALL_GITHUB_BASE_BRANCH=main
HEIMDALL_GITHUB_POLL_INTERVAL=30s
HEIMDALL_GITHUB_LOOKBACK_WINDOW=2m
HEIMDALL_GITHUB_APP_ID=your-app-id
HEIMDALL_GITHUB_INSTALLATION_ID=your-installation-id
...
```
.
├── cmd/heimdall/          # Main application entrypoint
├── internal/
