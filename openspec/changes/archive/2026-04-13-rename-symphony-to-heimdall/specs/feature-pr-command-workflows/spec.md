...
**Reason**: The command namespace is being rebranded from Symphony to Heimdall.
**Migration**: Replace `/symphony status` and `/symphony refine` usage with `/heimdall status` and `/heimdall refine` on Heimdall-managed pull requests.
