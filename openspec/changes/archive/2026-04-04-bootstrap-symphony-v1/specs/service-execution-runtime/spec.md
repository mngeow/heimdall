...
Heimdall MUST run OpenSpec and OpenCode workflows through local CLI execution on the Linux host where Heimdall is running, and it MUST verify that required local executables such as `git`, `openspec`, and `opencode` are available before the service reports readiness.

#### Scenario: Proposal generation is requested for an activated work item
- **WHEN** Heimdall begins a proposal workflow for a routed work item
