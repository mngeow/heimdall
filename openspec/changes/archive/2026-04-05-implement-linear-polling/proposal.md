...
Symphony's product and durable specs already depend on Linear polling as the trigger for proposal workflows, but the implementation contract is still underspecified. This change turns that expectation into an implementation-ready Linear GraphQL polling design that uses a static API key, project-level scope from `.env`, durable checkpoints, and safe retry behavior.

## What Changes

- Define the concrete Linear GraphQL polling contract against `https://api.linear.app/graphql` for Symphony's v1 board-provider adapter.
...
- Specify how Symphony pages through the `issues` connection, filters to the configured Linear project name from `.env`, orders by update time, and maps Linear issue fields into normalized work items.
...
- Require the v1 Linear polling configuration to include the project name in Symphony's dotenv schema.
