...
- [x] 2.2 Update the opencode execution adapter to parse `opencode run --format json`, capture the first structured-event `sessionID`, normalize structured events into sanitized timeline entries according to an explicit mapping, and preserve blocker or terminal events on the same command timeline.
- [x] 2.3 Wire the PR-command worker to create and update queued, starting, running, blocked, completed, and failed command-linked run state without requiring the dashboard to tail host logs.
...
- [x] 5.2 Run the relevant automated tests for the changed PR-command, opencode-runtime, dashboard, and behavior-test coverage, and verify they pass before considering the change complete.
