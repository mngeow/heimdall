...
**Reason**: The application and operator-facing configuration identity is being renamed from Symphony to Heimdall.
**Migration**: Replace Symphony-branded docs, service names, and configuration references with Heimdall-branded ones before running the renamed service.
...
### Requirement: Default local deployment paths use the Heimdall namespace
Heimdall MUST use Heimdall-branded default local deployment paths in its documented configuration and runtime defaults so fresh operator installs do not create new Symphony-named directories or database files.

#### Scenario: Operator relies on the documented default local paths
- **WHEN** an operator configures Heimdall by following the documented default local deployment layout
- **THEN** the documented default paths use `/etc/heimdall/`, `/var/lib/heimdall/`, and `/var/log/heimdall/`
- **AND** the documented default SQLite path uses `/var/lib/heimdall/state/heimdall.db`
