...
### Requirement: Bootstrap files use the Heimdall namespace
Heimdall MUST direct activation bootstrap repository scaffolding into the `.heimdall/bootstrap/` namespace so generated bootstrap artifacts align with the renamed workflow identity.

#### Scenario: Bootstrap prompt targets the canonical bootstrap file path
- **WHEN** Heimdall prepares bootstrap scaffolding for work item `ENG-123`
- **THEN** it instructs the local bootstrap execution to create or update `.heimdall/bootstrap/ENG-123.md`
- **AND** it does not continue using the legacy `.symphony/bootstrap/ENG-123.md` path for new bootstrap runs
