...
Symphony's current product and durable specs assume that a Linear activation event starts an OpenSpec proposal workflow: create a deterministic branch, scaffold an OpenSpec change, generate proposal artifacts, commit them, and open a pull request. The requested new flow is materially different. After an issue is observed in an active state, Symphony should extract the issue title and description, create a worktree from the configured local bare mirror, invoke the local `opencode` CLI with the general agent and model `gpt-5.4`, let that execution make a minimal arbitrary code change, commit and push the resulting branch, and open a pull request.

This is a cross-cutting workflow change. It affects Linear activation handling, repo worktree management, execution-runtime behavior, git mutation steps, GitHub branch push and PR creation, and the durable workflow semantics documented in the existing specs.

## Goals / Non-Goals

**Goals:**
- Replace the activation-driven OpenSpec proposal flow with a direct bootstrap PR workflow.
- Seed the bootstrap execution from the triggering issue's title and description.
- Create the worktree from the configured repository mirror path, make a non-empty repository change through local OpenCode execution, commit it, push it, and open or reuse a pull request.
...
**Non-Goals:**
- Generating or maintaining an OpenSpec change as part of the activation workflow.
...
- Keep OpenSpec artifact generation and run OpenCode only later. Rejected because the requested flow is a direct code-change bootstrap path.
...
- Replacing the OpenSpec-first path weakens the project's spec-driven posture -> Mitigation: make the workflow change explicit in docs and specs instead of pretending it is still an OpenSpec proposal flow.
...
- Arbitrary code changes may produce low-value or noisy PRs -> Mitigation: require a non-empty but minimal mutation and surface failures instead of forcing a PR when the result is poor.
...
1. Update the activation workflow to enqueue the new bootstrap PR workflow instead of the current proposal-artifact flow.
...
3. Update GitHub mutation logic to commit, push, and open or reuse PRs for the bootstrap flow.
4. Update product and workflow docs to describe the new activation behavior.
5. Validate with behavior tests that cover success, no-change failure, branch reconciliation, and PR creation.
