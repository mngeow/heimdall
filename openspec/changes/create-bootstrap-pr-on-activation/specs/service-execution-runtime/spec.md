...
- **WHEN** Symphony begins an activation-triggered bootstrap workflow for a routed work item
...
- **THEN** Symphony runs the local OpenCode execution by using the general agent and model `gpt-5.4`
...
Symphony MUST create the activation bootstrap worktree from the repository mirror configured by the resolved repository's local mirror path before invoking OpenCode.
...
- **THEN** Symphony uses that repository's configured local mirror path as the git source for the new worktree
...
Symphony MUST fail the activation bootstrap workflow when the OpenCode execution completes without producing any repository changes to commit.
...
- **THEN** Symphony does not create an empty commit, branch push, or pull request
