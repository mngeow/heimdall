...
### Requirement: Critical Symphony workflows have executable behavior coverage
Symphony MUST maintain executable behavior tests for the critical operator workflows: proposal creation from an activated work item, pull request refinement, agent-driven apply, unauthorized command rejection, and duplicate event safety.

#### Scenario: The behavior suite is run for a release candidate
- **WHEN** the Symphony behavior test suite is executed
...
Symphony behavior tests MUST be written in Gherkin `.feature` files, with exactly one `Feature` block per file and scenario names written in domain language.

#### Scenario: A new behavior area is added to the test suite
- **WHEN** a new end-to-end Symphony behavior area is introduced
...
Symphony behavior tests MUST use `Given` for initial context, `When` for actions or events, and `Then` for observable outcomes, with `And` and `But` used only as readable continuations.

#### Scenario: A workflow scenario is authored in Gherkin
- **WHEN** an author writes a Gherkin scenario for a Symphony workflow
...
Symphony behavior tests MUST assert observable outcomes such as branches, pull requests, comments, statuses, commits, or audit-visible results rather than depending solely on hidden internal state.

#### Scenario: A scenario verifies a rejected pull request command
- **WHEN** a behavior test checks how Symphony handles an unauthorized apply request
...
Symphony behavior tests MUST use `Background` only for shared context, MUST use `Scenario Outline` with `Examples` for data permutations, and MUST keep scenarios concise enough to remain readable as executable specifications.
...
Symphony MUST bind its Gherkin feature files to a Go-compatible test runner so behavior scenarios execute as part of the Go-based test suite or equivalent CI workflow.

#### Scenario: A new feature file is added for pull request refine behavior
- **WHEN** a new Gherkin feature file is introduced for Symphony behavior coverage
