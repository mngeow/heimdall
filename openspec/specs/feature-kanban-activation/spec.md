...
### Requirement: Active state transitions start proposal workflows
Symphony MUST detect when a configured work item transitions into the normalized `active` lifecycle bucket and start a `propose` workflow for the mapped repository.
...
- **THEN** Symphony creates a workflow run of type `propose`
...
- **THEN** Symphony does not create a branch, OpenSpec change, or pull request
...
Symphony MUST reconcile existing bindings before creating a new branch, change, or pull request for the same work item and repository.
...
- **THEN** Symphony reuses the existing branch, change, and pull request binding
