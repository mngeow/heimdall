...
This folder explains how to prepare a Linux host, GitHub, and Linear so Heimdall can run end-to-end.
...
- Heimdall is designed for one Linux host and one operator-owned deployment.
...
4. Create a dedicated Linear account for Heimdall and generate its API key.
5. Copy `dist.env` to the project-root `.env` file and fill in the required values.
6. Start Heimdall under `systemd`.
